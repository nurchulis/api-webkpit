...
class Api_artikel extends RestController {
    ...
    function index_get() { 
        ...
        if ($id_artikel == '') { //Cek Jika id_artikel nya kosong
            if($parameter=='show_terbaru'){//5. Menampilkan Artikel terbaru berasarkan data tgl_edit artikel, 6 artikel
                $this->db->order_by('tgl_edit', 'DESC'); //Menampilkan data Artikel yang terbaru dengan tgl_edit
                $this->db->limit(6);
                $artikel= $this->db->get('artikel')->result();
            }else if($parameter=='show_populer') {//6. Menampilkan Artikel Terpopuler Sebanyak 6 Artikel
                $this->db->order_by('log_kunjungan', 'DESC'); // Function SQL untuk Menampilkan Data artikel yang memiliki log_kunjungan Paling banyak 
                $this->db->limit(6);
                $artikel = $this->db->get('artikel')->result();
            }else {
                $this->db->order_by('tgl_edit', 'DESC'); // Function SQL untuk Menampilkan Data terbaru dengan tgl_edit
                $artikel = $this->db->get('artikel')->result(); //1. Menampilkan semua artikel
            }
        } else { //Jika ada id_artikel Maka lanjut
            if($parameter=='hapus'){//3. Menghapus artikel berdasarkan id_artikel
                ...
                if($artikel){    
                    ...
                    $this->hapus_gambar($file_gambar);  //Menghapus File Gambar yang ada dalam Folder Thumbnail dan Artikel
                    //-----------------------------------------//
                    $this->db->where('id_artikel', $id_artikel); //Function Untuk Menghapus berdasarkan id_arikel
                    ...
                    $this->response(array('status' => 'failed','Description' => 'id_artikel salah gan atau data sudah dihapus'), 201);
                }
        ...
        if($parameter == 'update'){  //DIPILIH UPDATE
            ...
            if($artikel){
                ...
                if($_FILES["gambar"]["name"] == '') {
                    ...
                    $old_file =($artikel[0]->gambar); //Nama file Sebelumnya
                  ...
                  $this->response(array('status' => 'failed','Description'=> 'id_artikel tidak ada'), 201);
                }        
            }else {
    ...
    function hapus_gambar($old_file){ //Menghapus gambar lama saat proses update
       ...
       // $old_file_thumb=$this->merge($old_file,'_thumb'); //Simpan File Thumbnail di folder Server
        $file_thumb = './Assets/img/artikel/thumbnail/'.$old_file;
        unlink($file_thumb);
    }
